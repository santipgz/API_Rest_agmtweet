...
// Sacar una publicacion concreta por id
const detail = (req, res) => {
    ...
    Publication.findById(publicationId, (error, publicationStored) => {
        ...
        return res.status(200).send({
            status: "success",
            message: "Mostrar publicacion por id",
            publication: publicationStored
        });
    });
}

// Eliminar publicacion concreta por id, solo puede borrar una publicacion el usuario que este identificado con el token
...
const upload = (req, res) => {
    ...
    Publication.findOneAndUpdate({ "user": req.user.id, "_id": publicationId }, { file: req.file.filename }, { new: true }, (error, publicationUpdated) => {
        if (error || !publicationUpdated) {
            return res.status(500).send({
                status: "error",
                message: "Error en la subida del archivo a la publicacion"
            })
        }
...
// Listar todas las publicaciones (FEED) - de todos los usuarios que un usuario identificado esta siguiendo
